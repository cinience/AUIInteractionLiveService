<template>
  <div class="page">
    <div class="page-nav">
      ...
      <div class="func-list">
        <el-button type="text" @click="$router.replace('/live/MobileDownload')"
          >下载移动端</el-button
        >
    ...
    <div class="page-content">
      <div class="title">
        <i
          class="el-icon-back"
          @click="() => $router.replace('/live/liveList')"
          style="cursor: pointer"
        ></i>
        直播标题：{{ liveDetail.title }}
      </div>
      <div class="btn-container">
        <el-button type="primary" size="small" @click="updateLive"
          >更新直播信息</el-button
        >
        <el-button
          type="success"
            ...
            <div class="item">
              <div class="content">
                ...
                <el-button
                  ...
                  type="primary"
                  class="copy"
                  @click="() => copyPush(pushLink)"
                  >复制</el-button
                >
              </div>
              <div class="push" @click="jumpPC">PC主播端推流</div>
            </div>
          </div>
          <el-button size="small" slot="reference">PC主播端推流</el-button>
        </el-popover>
        <!-- <el-button size="small" @click="introLiveRoom(liveDetail.liveId)">进入直播间</el-button> -->
        <el-popover
          id="popover"
          placement="bottom"
          width="380"
        ...
        >
          ...
          <el-button slot="reference" size="small"
            >观看地址 <i class="el-icon-caret-bottom"></i
          ></el-button>
        </el-popover>
        <el-checkbox
          v-model="checked"
      ...
      <div class="main-container">
        <div class="player-container">
          ...
          <div class="player">
            <div class="not-play" v-show="!playing">
              <span>{{
                liveDetail.status === 2 ? "点击播放直播回放" : "直播还未开始"
              }}</span>
            </div>
            <div v-if="playback" class="stop-play" @click="startPlayback">
              <img
        ...
        <div class="chat-container">
          ...
          >
            <el-tab-pane
              ...
              name="message"
              :class="`message ${
                liveDetail.notice ? 'notice-hoverd-message' : ''
              }`"
            >
              <chat
                ref="chat"
                :parentIsMuteAll="isMuteAll"
                :notice="liveDetail.notice"
                :commentList="commentList"
                :chatService="chatService"
                :banList="banList"
                @kickUserShort="kickUserShort"
                @kickUserPermanently="kickUserPermanently"
                @banComment="handleBanComment"
              />
            </el-tab-pane>
    ...
    <el-dialog title="商品列表" :visible.sync="goodVisible" width="500px">
      <div class="good-list">
    ...
    <el-dialog title="公告" :visible.sync="noticeVisible" width="500px">
      <el-input
        type="textarea"
        placeholder="请输入内容"
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button
          type="primary"
          @click="deleteNoticeHandler"
          size="small"
          v-if="!!liveDetail.notice"
          >下 架</el-button
        >
        <el-button
          type="primary"
          @click="updateNoticeHandler"
          size="small"
          v-else
          >发 布</el-button
        >
...
export default {
  ...
  methods: {
    ...
    kickUserShort(kickuserId) {
      liveApi
        .kickUser({
          appId: this.$store.getters.appId,
          roomId: this.liveDetail.roomId,
          userId: this.userId,
          kickUser: kickuserId,
        })
        .then((res) => {
          if (res.status === 200) {
            this.$message.success('踢出房间');
            this.kicked = '';
            this.kickUserVisible = false;
          }
        });
    },
    kickUserPermanently(kickuserId) {
      liveApi
        .kickUser({
          appId: this.$store.getters.appId,
          roomId: this.liveDetail.roomId,
    ...
    getShareUrl(liveId) {
      liveApi
        ...
        .then((res) => {
          this.shareLink = `${window.location.protocol}//${res.data.result.absolutePath}`;
        });
    },
    getAnchorUrl(liveId) {
      liveApi
        ...
        .then((res) => {
          this.pushLink = `${res.data.result.absolutePath}`;
        });
    },
    // introLiveRoom(liveId) {
    ...
    getLiveRoom() {
      return liveApi.getLiveRoom({ id: this.liveId }).then((res) => {
        ...
        this.playback = this.liveDetail.status === 2;
        this.countTime
          = this.liveDetail.status === 1
            ? +new Date() - this.liveDetail.startTime
            : this.liveDetail.endTime - this.liveDetail.startTime;
        return res.data;
      });
    },
    ...
    jumpPC() {
      liveApi
        .getStandardRoomJumpUrl({
          bizId: this.liveId,
          userId: this.updateLiveModel.anchorId,
          userNick: this.updateLiveModel.anchorNick,
    ...
    async startPlayback() {
      try {
        this.liveService.stopPlay();
        const liveDetail = await this.roomEngine.getLiveDetail(this.liveId);
        let currentProtocol = 'https';
        if (window.location.protocol.indexOf('https') === -1) {
          currentProtocol = 'http';
        }
        console.log(liveDetail, '===liveDetail===');
        if (liveDetail.vodInfo.playUrl) {
          this.liveService.setPlayerConfig({
            container: '#player',
            width: '100%',
            height: '100%',
            isLive: false,
    ...
    async handleBanComment() {
      this.banList = (
        await this.chatService.listBanCommentUsers(1, 50)
      ).banCommentUserModelList.map((item) => item.userId);
    },
    ...
    bindEvents() {
      ...
      this.chatService.on(
        EventNameEnum.PaaSChatReceiveComment,
      ...
      this.liveService.on(EventNameEnum.PaaSLiveStreamStart, () => {
        this.$message.info('推流开始');
      });
      this.liveService.on(EventNameEnum.PaaSLiveStreamStop, () => {
        this.$message.info('推流暂停');
      });
    ...
    async auth(roomId, status) {
      ...
      try {
        ...
        const commentList = (await this.chatService.listComment(1, 1, 50))
          .commentModelList;
  ...
  created() {
    ...
    this.getLiveRoom().then((res) => {
      ...
      this.getShareUrl(this.liveId);
      this.getAnchorUrl(this.liveId);
    });
  },
